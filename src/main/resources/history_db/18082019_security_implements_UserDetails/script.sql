...
-- -----------------------------------------------------
-- Table `lanit`.`user_accounts`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `lanit`.`user_accounts` (
  `id_user` INT UNSIGNED NOT NULL,
  `username` VARCHAR(50) NOT NULL,
  `password` VARCHAR(100) NOT NULL,
  `enabled` TINYINT(0) NOT NULL,
  `accountNonExpired` TINYINT(0) NOT NULL,
  `accountNonLocked` TINYINT(0) NOT NULL,
  `credentialsNonExpired` TINYINT(0) NOT NULL,
  PRIMARY KEY (`id_user`),
  CONSTRAINT `id_user`
    FOREIGN KEY (`id_user`)
    REFERENCES `lanit`.`users` (`id`)
...
CREATE UNIQUE INDEX `username_UNIQUE` ON `lanit`.`user_accounts` (`username` ASC) INVISIBLE;


-- -----------------------------------------------------
-- Table `lanit`.`user_accounts_authorities`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `lanit`.`user_accounts_authorities` (
  `id` INT UNSIGNED NOT NULL AUTO_INCREMENT,
  ...
  CONSTRAINT `id_account`
    ...
    REFERENCES `lanit`.`user_accounts` (`id_user`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;

CREATE UNIQUE INDEX `authority_id_account` ON `lanit`.`user_accounts_authorities` (`id_account` ASC, `authority` ASC) INVISIBLE;
...
START TRANSACTION;
USE `lanit`;
INSERT INTO `lanit`.`notifications` (`id`, `id_org`, `notification_type`, `id_notification_status`, `date_received`, `date_response`, `letter_number`, `id_user_curator_gos`, `id_user_implementor`) VALUES (1, 7, 'Уведомление о непубликации сведений об оплате', 1, '2014-07-05', '2014-07-15', '123456789012', 5, 1);
INSERT INTO `lanit`.`notifications` (`id`, `id_org`, `notification_type`, `id_notification_status`, `date_received`, `date_response`, `letter_number`, `id_user_curator_gos`, `id_user_implementor`) VALUES (2, 11, 'Уведомление о незакрытии контракта', 2, '2015-07-05', '2015-07-05', '123456789012', 8, 6);
INSERT INTO `lanit`.`notifications` (`id`, `id_org`, `notification_type`, `id_notification_status`, `date_received`, `date_response`, `letter_number`, `id_user_curator_gos`, `id_user_implementor`) VALUES (3, 12, 'Уведомление о невключении поставщика в реестр недобросовестных поставщиков', 1, '2016-06-06', '2016-06-07', '123456789012', 10, 6);
INSERT INTO `lanit`.`notifications` (`id`, `id_org`, `notification_type`, `id_notification_status`, `date_received`, `date_response`, `letter_number`, `id_user_curator_gos`, `id_user_implementor`) VALUES (4, 11, 'Уведомление о заключении контракта с ед. поставщиком по п. 6 ч.1 ст.93', 1, '2014-07-05', '2014-07-06', '123456789012', 9, 7);

COMMIT;


-- -----------------------------------------------------
-- Data for table `lanit`.`user_accounts`
-- -----------------------------------------------------
START TRANSACTION;
USE `lanit`;
INSERT INTO `lanit`.`user_accounts` (`id_user`, `username`, `password`, `enabled`, `accountNonExpired`, `accountNonLocked`, `credentialsNonExpired`) VALUES (1, 'user1', '$2a$10$aI2YkjH3eUZcCdn.9cH0Qu8ZUQAxwKWaprhAniiCwDVFcvmqRO8X.', true, true, true, true);
INSERT INTO `lanit`.`user_accounts` (`id_user`, `username`, `password`, `enabled`, `accountNonExpired`, `accountNonLocked`, `credentialsNonExpired`) VALUES (2, 'user2', '$2a$10$aI2YkjH3eUZcCdn.9cH0Qu8ZUQAxwKWaprhAniiCwDVFcvmqRO8X.', true, true, true, true);

COMMIT;


-- -----------------------------------------------------
-- Data for table `lanit`.`user_accounts_authorities`
-- -----------------------------------------------------
START TRANSACTION;
USE `lanit`;
INSERT INTO `lanit`.`user_accounts_authorities` (`id`, `id_account`, `authority`) VALUES (1, 1, 'ROLE_ADMIN');
INSERT INTO `lanit`.`user_accounts_authorities` (`id`, `id_account`, `authority`) VALUES (2, 2, 'ROLE_USER');
